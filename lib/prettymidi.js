...
exports.once = function (files, dest) {
  ...
  files.forEach(function (file) {
    ...
    fs.stat(file, function (err, stats) {
      if (err) { throw err; }
      if (stats.isFile()) {
        exports.parse(file, dest);
      } else if (stats.isDirectory()) {
        utils.log("directory parsing not implemented yet");
      }
    });
  });
};
...
exports.parse = function (file, dest) {
  ...
  fs.readFile(file, function (err, data) {
    ...
    var result = JSON.stringify(exports.decode(data), null, 2)
      , fname = path.basename(file, ".mid") + ".json"
