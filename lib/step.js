...
exports.decode = function (buffer) {
  var data = new DataView(buffer)
    , index = 0
    , invalidHeader = false
        || (data.getUint8(index++) != 0x4d) 
        || (data.getUint8(index++) != 0x54) 
        || (data.getUint8(index++) != 0x68) 
        || (data.getUint8(index++) != 0x64);
  ...
  if (data.getUint32(index) !== 6) {
    utils.error("file header length is not six.");
    process.exit();
  }
  ...
  midiFile.format = data.getUint16(index);

  index += 2;

  if ((midiFile.format < 0) || (midiFile.format > 2)) {
    utils.error("MIDI file format " + midiFile.format + " unrecognized.");
  }

  if (midiFile.format == 2) {
    utils.error("MIDI file format type 2 not supported.");
  }

  //   unsigned short numTracks;
  midiFile.numTracks = data.getUint16(index);

  index += 2;

  //   unsigned short ticksPerBeat;
  midiFile.ticksPerBeat = data.getUint16(index);
  index+=2;

  midiFile.tracks=new Array(midiFile.numTracks);
  for (iTrack=0; iTrack<midiFile.numTracks; iTrack++) {
    midiFile.tracks[iTrack] = {};
    index = decodeTrack( data, midiFile.tracks[iTrack], index);
    if (index == -1)
